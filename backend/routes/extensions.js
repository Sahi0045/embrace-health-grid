...
export function registerExtensionRoutes(app, deps) {
  ...
  app.post(
    ...
    hipaaAuditPHIAccess("Prescription"),
    requireRole("staff", "admin", "doctor"),
    (req, res) => {
  ...
  app.post("/api/nfc/issue", requireRole("admin"), hipaaAuditPHIAccess("NFCCard"), (req, res) => {
    ...
    solana.scheduleAnchor({ computeRecordHash }, card, "nfc-issue", req.user.did || null);
  ...
  app.post("/api/nfc/verify", requireRole("staff", "admin"), hipaaAuditPHIAccess("NFCCard"), (req, res) => {
    ...
    if (cardId) {
      ...
      if (!entry) {
        logAudit(req, {
          resource: cardId,
          action: "NFC_VERIFY_FAILED",
          outcome: "failure",
          severity: "warning",
        });
        return res
          .status(404)
          .json({ error: "Card not found in registry", code: "CARD_NOT_FOUND" });
      }
      card = entry.value;
      ...
      if (!result.valid) {
        const errorCode =
          result.error && result.error.includes("expired") ? "DID_EXPIRED" : "SIGNATURE_INVALID";
        logAudit(req, {
          resource: payload.did || "unknown",
          action: "NFC_VERIFY_FAILED",
          outcome: "failure",
          severity: "warning",
        });
      ...
      if (cardEntries.length === 0) {
        ...
        return res
          .status(404)
          .json({ error: "No NFC card registered for this patient", code: "CARD_NOT_FOUND" });
      }

      // Sort by issuedAt descending to get the latest card
      cardEntries.sort(
        (a, b) => new Date(b.value.issuedAt).getTime() - new Date(a.value.issuedAt).getTime(),
      );
      const latestCard = cardEntries[0].value;

      if (latestCard.status === "revoked") {
        logAudit(req, {
          resource: latestCard.cardId,
          action: "NFC_VERIFY_FAILED",
          outcome: "failure",
          severity: "warning",
        });
        return res
          .status(403)
          .json({ error: "Card has been revoked", code: "CARD_REVOKED", card: latestCard });
      }
  ...
  app.post(
    ...
    hipaaAuditPHIAccess("InsuranceClaim"),
  ...
  app.post(
    "/api/vaccines",
    hipaaAuditPHIAccess("VaccinationRecord"),
    requireRole("staff", "admin"),
    (req, res) => {
      const {
        patientDid,
  ...
  app.get("/api/inpatient/:patientDid", hipaaAuditPHIAccess("InpatientRecord"), (req, res) => {
    ...
    res.json({
      admission:
        admissions.length > 0
          ? admissions.sort((a, b) =>
              (b.value.admissionDate || "").localeCompare(a.value.admissionDate || ""),
            )[0].value
          : null,
  ...
  // ─── Health Metrics ─────────────────────────────────────────────────────────
  app.get(
    "/api/medical-records/:patientDid/metrics",
    hipaaAuditPHIAccess("HealthMetrics"),
    consentGate,
    (req, res) => {
      if (req.user.role === "patient" && req.user.did !== req.params.patientDid) {
        return res.status(403).json({ error: "Forbidden" });
        ...
        const defaultMetrics = [
          {
            ...
            weight: 68.5,
            bmi: 25.2,
            bloodSugar: { fasting: 112, postMeal: 148 },
            bloodPressure: { systolic: 138, diastolic: 88 },
            cholesterol: { total: 210, hdl: 42, ldl: 142 },
            hba1c: 7.2,
            patientDid: req.params.patientDid,
          },
          {
            ...
            weight: 69.2,
            bmi: 25.4,
            bloodSugar: { fasting: 118, postMeal: 154 },
            bloodPressure: { systolic: 142, diastolic: 90 },
            cholesterol: { total: 218, hdl: 40, ldl: 150 },
            hba1c: 7.4,
            patientDid: req.params.patientDid,
          },
          {
            ...
            weight: 70.1,
            bmi: 25.8,
            bloodSugar: { fasting: 124, postMeal: 162 },
            bloodPressure: { systolic: 145, diastolic: 92 },
            cholesterol: { total: 225, hdl: 38, ldl: 158 },
            hba1c: 7.6,
            patientDid: req.params.patientDid,
          },
          {
            ...
            weight: 70.8,
            bmi: 26.0,
            bloodSugar: { fasting: 130, postMeal: 170 },
            bloodPressure: { systolic: 148, diastolic: 95 },
            cholesterol: { total: 232, hdl: 36, ldl: 166 },
            hba1c: 7.9,
            patientDid: req.params.patientDid,
          },
          {
            ...
            weight: 71.5,
            bmi: 26.3,
            bloodSugar: { fasting: 135, postMeal: 178 },
            bloodPressure: { systolic: 152, diastolic: 98 },
            cholesterol: { total: 240, hdl: 35, ldl: 175 },
            hba1c: 8.2,
            patientDid: req.params.patientDid,
          },
        ];
  ...
  // ─── Pharmacy Orders ────────────────────────────────────────────────────────
  app.get(
    "/api/pharmacy-orders/:patientDid",
    hipaaAuditPHIAccess("PharmacyOrder"),
    consentGate,
    (req, res) => {
      if (req.user.role === "patient" && req.user.did !== req.params.patientDid) {
        return res.status(403).json({ error: "Forbidden" });
  ...
  // ─── Rehab Sessions ─────────────────────────────────────────────────────────
  app.get(
    "/api/rehab-sessions/:patientDid",
    hipaaAuditPHIAccess("RehabSession"),
    consentGate,
    (req, res) => {
      if (req.user.role === "patient" && req.user.did !== req.params.patientDid) {
        return res.status(403).json({ error: "Forbidden" });
  ...
  // ─── Feedback List ──────────────────────────────────────────────────────────
  app.get(
    "/api/feedback/:patientDid",
    hipaaAuditPHIAccess("PatientFeedback"),
    consentGate,
    (req, res) => {
      if (req.user.role === "patient" && req.user.did !== req.params.patientDid) {
        return res.status(403).json({ error: "Forbidden" });
  ...
  app.post("/api/staff-requests", requireRole("staff", "admin"), (req, res) => {
    const { requestType, leaveType, fromDate, toDate, reason, shiftDate, shiftType, unit } =
      req.body;
  ...
  // ─── Staff Schedule (per-doctor isolation) ──────────────────────────────────
  ...
  // ─── Insurance Policies ─────────────────────────────────────────────────────
  app.get(
    "/api/insurance/policies/:patientDid",
    hipaaAuditPHIAccess("InsurancePolicy"),
    (req, res) => {
  ...
  app.post("/api/preferences/:patientDid", (req, res) => {
    ...
    const preferences = {
      ...
      insuranceVerification:
        typeof insuranceVerification === "boolean" ? insuranceVerification : true,
      ...
      const defaultShifts = [
        {
          ...
          unit: "Cardiology OPD",
          patients: 24,
          notes: "Routine consultations + 4 new referrals",
          confirmed: true,
        },
        {
          id: "s2",
          ...
          unit: "Cardiology Ward 4A",
          patients: 12,
          notes: "Post-cath follow-ups",
          confirmed: true,
        },
        {
          id: "s3",
          ...
          unit: "Virtual Clinic",
          patients: 8,
          notes: "4 teleconsults + 4 review calls",
          confirmed: true,
        },
        {
          id: "s4",
          ...
          unit: "OR Suite 3",
          patients: 2,
          notes: "CABG × 1, Pacemaker implant × 1",
          confirmed: true,
        },
        {
          id: "s5",
          ...
          unit: "ICU Block B",
          patients: 6,
          notes: "Critical monitoring + 2 new admissions",
          confirmed: true,
        },
        {
          id: "s6",
        ...
        },
        {
          ...
          unit: "Emergency + Cardio",
          patients: null,
          notes: "Night on-call. Emergency pager active.",
          confirmed: true,
        },
        {
          id: "s8",
      ...
      const defaultPolicies = [
        {
          ...
          description:
            "Enforce patient consent verification for all electronic medical records access requests by physicians.",
        },
        {
          id: "p2",
          ...
          updatedAt: "2026-06-08",
          description:
            "Every read and write access on medical records must be immutably anchored on the decentralized ledger.",
        },
      ];
