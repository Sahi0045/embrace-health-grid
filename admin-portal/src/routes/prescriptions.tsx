...
async function apiFetch<T>(path: string): Promise<T> {
  ...
  const clientKey = (typeof import.meta !== "undefined" && import.meta.env?.VITE_CLIENT_KEY)
...
function AdminPrescriptionsPage() {
  ...
  const withReports = prescriptions.filter((rx) => allRecords.some((r) => r.rxId === rx.rxId)).length;
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8 space-y-6">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-4 py-2.5 text-xs text-primary">
        ...
        <span className="font-medium">Read-only audit view — Admin can view but cannot create, edit, or delete prescriptions or medical reports.</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {consultations.map((cx) => {
            ...
            return (
              <div key={cx.rxId} className="rounded-xl border border-border bg-card shadow-clinical overflow-hidden">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-foreground flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${sCls}`}>{cx.status ?? "active"}</span>
                          {cx.linkedReport && (
                        ...
                        <div className="text-xs text-muted-foreground">
                          Dx: {cx.diagnosis || "—"}{cx.chiefComplaint ? ` · ${cx.chiefComplaint}` : ""}
                        </div>
                        <div className="flex items-center gap-3 mt-0.5 text-[10px] text-muted-foreground flex-wrap">
                          <span className="flex items-center gap-1"><User className="h-3 w-3" />{cx.doctorName || cx.signedBy || "—"}</span>
