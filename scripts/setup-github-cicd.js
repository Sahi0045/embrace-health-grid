...
async function main() {
  ...
  try {
    const { data: sk } = await gh('GET', `/repositories/${repoId}/environments/staging/secrets/public-key`);
    stagingKey = sk;
  ...
  try {
    const { data: pk } = await gh('GET', `/repositories/${repoId}/environments/production/secrets/public-key`);
    productionKey = pk;
  ...
  for (const [name, value] of Object.entries(stagingSecrets)) {
    if (!value) {
      manualSecrets.push({ env: 'staging', name, description: `Set this to your actual staging ${name}` });
  ...
  for (const [name, value] of Object.entries(productionSecrets)) {
    if (!value) {
      manualSecrets.push({ env: 'production', name, description: `Set this to your actual production ${name}` });
  ...
  if (runs.workflow_runs?.length) {
    ...
    log('  ', `Commit: ${latest.head_sha.substring(0, 8)} — ${latest.head_commit?.message?.split('\n')[0]}`);
  ...
  if (manualSecrets.length > 0) {
    ...
    console.log('   Go to: https://github.com/Sahi0045/embrace-health-grid/settings/environments\n');
