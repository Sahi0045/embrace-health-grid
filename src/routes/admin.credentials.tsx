...
export const Route = createFileRoute("/admin/credentials")({
  ...
  component: CredentialsPageGuarded,
});
  ...
  const displayCredentials: CredentialFull[] = (credentialsData?.credentials ?? []).map(
    (c: ApiCredential) => {
      return {
        ...
        issuedAt: String(c.issuedAt ?? "").split("T")[0] || new Date().toISOString().split("T")[0],
        expiresAt:
          String(c.expiresAt ?? "").split("T")[0] || new Date().toISOString().split("T")[0],
        // Postgres uses 'valid'; this view models active|expired|revoked.
        status: c.status === "revoked" || c.status === "expired" ? c.status : ("active" as const),
        schema: `https://schema.did-hospital.in/v1/${(c.type || "").toLowerCase()}`,
        verificationCount: 1,
        lastVerified:
          String(c.issuedAt ?? "").split("T")[0] || new Date().toISOString().split("T")[0],
      };
    },
  );
 ...
 */
function CredentialsPageGuarded() {
  return (
    <RouteGuard requiredRole="admin">
      <CredentialsPage />
    </RouteGuard>
