...
const CATEGORIES = ["access", "consent", "credential", "infrastructure", "auth", "prescription", "emergency"] as const;
    ...
    <div className="min-h-screen">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value as Category | "all"); setPage(0); }}
            className="rounded-xl border border-border bg-card px-3 py-2.5 text-sm text-foreground outline-none">
            <option value="all">All Categories</option>
            {CATEGORIES.map(c => <option key={c} value={c} className="capitalize">{c}</option>)}
          </select>

          <select value={severityFilter} onChange={(e) => { setSeverityFilter(e.target.value as typeof severityFilter); setPage(0); }}
            className="rounded-xl border border-border bg-card px-3 py-2.5 text-sm text-foreground outline-none">
          ...
          <select value={resultFilter} onChange={(e) => { setResultFilter(e.target.value as typeof resultFilter); setPage(0); }}
            className="rounded-xl border border-border bg-card px-3 py-2.5 text-sm text-foreground outline-none">
        ...
        <div className="text-xs text-muted-foreground">
          Showing {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, filtered.length)} of {filtered.length.toLocaleString()} events
        </div>

        {/* Events */}
        <div className="space-y-3">
          {paginated.map(event => <AuditEventCard key={event.id} event={event} />)}
        </div>
