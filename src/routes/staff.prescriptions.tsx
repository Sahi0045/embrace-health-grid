...
function PrescriptionsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [rxRes, recRes] = await Promise.all([getAllPrescriptions(), getAllMedicalRecords()]);
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-4 py-2.5 text-xs text-primary">
          ...
          <span className="font-medium">
            Read-only — Staff can view prescriptions and medical reports but cannot create, modify,
            or delete them.
          </span>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 text-center">
          {[
            ...
            {
              label: "With Reports",
              value: prescriptions.filter((r) => allRecords.some((rec) => rec.rxId === r.rxId))
                .length,
              cls: "text-chart-2",
            },
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-semibold text-foreground flex items-center gap-2 flex-wrap">
                              ...
                              <span
                                className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${statusCls}`}
                              >
                                {cx.status || "active"}
                              </span>
                              {cx.linkedReport && (
                            ...
                            <div className="flex items-center gap-3 mt-0.5 text-[10px] text-muted-foreground flex-wrap">
                              <span className="flex items-center gap-1">
                                ...
                                {cx.doctorName || cx.signedBy || "Doctor"}
                              </span>
                              <span className="flex items-center gap-1">
                    ...
                    {isExp && (
                      <div className="border-t border-border px-4 pb-5 pt-3 space-y-4">
                        ...
                        <div className="flex items-center gap-2 rounded-lg border border-warning/30 bg-warning/5 px-3 py-2 text-xs text-muted-foreground">
                          ...
                          Read-only — Staff cannot modify prescriptions or reports issued by
                          doctors.
                        </div>
