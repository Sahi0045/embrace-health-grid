...
const credentialTimeline = [
  { id: "ct1", action: "issued" as const, label: "Credential issued by Apollo Hospitals", issuer: "Apollo Hospitals", at: "2025-01-12" },
  { id: "ct2", action: "verified" as const, label: "Verified by Star Health Insurance", issuer: "Star Health", at: "2025-03-08" },
  { id: "ct3", action: "verified" as const, label: "Verified during hospital admission", issuer: "Apollo ER Desk", at: "2025-11-22" },
  { id: "ct4", action: "verified" as const, label: "Cross-hospital verification via AIIMS", issuer: "AIIMS Delhi", at: "2026-02-14" },
];
          ...
          <div className="lg:col-span-3 space-y-3">
            ...
            {filtered.length === 0 && (
              <div className="rounded-xl border border-dashed border-border p-12 text-center">
                ...
                <div className="text-sm text-muted-foreground">No credentials match your filters</div>
              </div>
            )}
          </div>
            ...
            <AnimatePresence mode="wait">
              {selected ? (
                <motion.div key={selected.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                  <div className="rounded-xl border border-border bg-card p-5 shadow-clinical">
                    <div className="flex items-center justify-between mb-4">
                      <div className="text-sm font-semibold text-foreground">{selected.typeLabel}</div>
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${selected.status === "active" ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}>
                        {selected.status}
                      </span>
                    </div>
                    ...
                    <div className="space-y-3 text-xs">
                      ...
                      <div>
                        ...
                        <div className="font-mono text-[10px] text-muted-foreground/70 break-all">{selected.holderDID}</div>
                      </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-xl border border-dashed border-border p-10 text-center">
                  ...
                  <div className="text-sm text-muted-foreground">Select a credential to inspect its details and audit trail</div>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
