...
function Consent() {
  ...
  const decide = (id: string, approve: boolean) => {
    setList((prev) => prev.map((c) => (c.id === id ? { ...c, status: approve ? "active" : "revoked" } : c)));
            ...
            return (
              <div key={c.id} className="rounded-2xl border border-border bg-card p-4 shadow-clinical">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="font-semibold text-foreground">{c.requester}</div>
                    <div className="text-sm text-muted-foreground">{c.requesterRole}</div>
                  </div>
                  ...
                  {status === "pending" ? (
                    <div className="flex w-full gap-2">
                      <button
                        ...
                        className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-border bg-card px-3 py-2 text-sm font-medium hover:bg-muted"
                      >
