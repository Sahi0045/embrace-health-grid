...
import {
  Download, Search, Filter, Eye, FileSignature, PencilLine,
...
type AuditAction = "viewed" | "signed" | "exported" | "updated" | "created" | "deleted" | "revoked" | "verified";

const actionConfig: Record<AuditAction, { bg: string; text: string; icon: React.ComponentType<{className?: string}> }> = {
...
function AuditLogs() {
  ...
  const { data: fabricData, online, loading: fabricLoading, refetch } = useFabricAudit(page);

  // Merge: live Fabric events first, then extra entries, then mock events
  const fabricEvents = (fabricData?.events ?? []) as Array<{ actor?: string; resource?: string; action?: string; loggedAt?: string; category?: string; txId?: string }>;
  const liveEntries = fabricEvents.map((e, i) => ({
    id: e.txId ?? `fab_${i}`,
  ...
  const filtered = useMemo(() => {
    ...
    if (query) {
      ...
      rows = rows.filter(e => e.actor.toLowerCase().includes(q) || e.resource.toLowerCase().includes(q) || e.actorRole.toLowerCase().includes(q) || e.action.toLowerCase().includes(q));
    }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2 items-center">
            ...
            <button
              onClick={() => fabricLogAuditEvent("Admin Console", "Audit export", "exported", "success", "info").catch(() => {})}
      ...
      <div className="space-y-5 p-6 sm:p-8">
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {summaryStats.map(s => {
            const Icon = s.icon;
            return (
              <motion.div key={s.label} variants={fadeUp} className="rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        {/* Integrity banner */}
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex items-center gap-3 rounded-xl border border-success/30 bg-success/6 px-4 py-3">
          <Database className="h-5 w-5 text-success shrink-0" />
          <div className="flex-1 text-xs text-foreground">
            <span className="font-semibold">Ledger integrity verified.</span> Last block: <span className="font-mono text-success">#1,285,044</span> · Merkle root: <span className="font-mono text-muted-foreground">0x4f8a…c9d1</span> · <span className="text-muted-foreground">Verified {new Date().toLocaleTimeString()}</span>
          </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="flex flex-1 items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 shadow-sm">
            ...
            <input value={query} onChange={e => { setQuery(e.target.value); setPage(0); }} placeholder="Search actor, resource, action…" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
          </div>
          <div className="flex gap-2 flex-wrap">
            <select value={actionFilter} onChange={e => { setActionFilter(e.target.value as AuditAction | "all"); setPage(0); }} className="rounded-lg border border-border bg-card px-3 py-2 text-sm outline-none">
              <option value="all">All Actions</option>
              {(Object.keys(actionConfig) as AuditAction[]).map(a => <option key={a} value={a}>{a}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="flex gap-1 border-b border-border">
          {categories.map(c => (
            <button key={c} onClick={() => { setCategory(c); setPage(0); }} className={`px-3 py-2 text-xs font-medium border-b-2 transition-colors ${category === c ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
              {c}
            </button>
          ))}
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-left text-[11px] uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="px-4 py-3 font-semibold cursor-pointer hover:text-foreground" onClick={() => toggleSort("at")}>
                    Time <SortIcon field="at" />
                  </th>
                  <th className="px-4 py-3 font-semibold cursor-pointer hover:text-foreground" onClick={() => toggleSort("actor")}>
                    Actor <SortIcon field="actor" />
                  </th>
                  <th className="px-4 py-3 font-semibold cursor-pointer hover:text-foreground" onClick={() => toggleSort("action")}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="font-medium text-foreground whitespace-nowrap">{e.actor}</div>
                            <div className="text-[10px] text-muted-foreground">{e.actorRole}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${cfg.bg} ${cfg.text}`}>
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{e.category}</span>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell font-mono text-[10px] text-muted-foreground">
                        0x{e.id.slice(-4).padEnd(4,"0")}…{(parseInt(e.id.replace(/\D/g,"") || "1",10) * 7).toString(16).slice(0,4)}
                      </td>
                    </motion.tr>
                  );
                })}
        ...
        <div className="flex items-center justify-between">
          <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="rounded-lg border border-border bg-card px-3 py-2 text-sm disabled:opacity-40 hover:bg-muted">
            ← Previous
          </button>
          <div className="text-xs text-muted-foreground">Page {page + 1} of {totalPages || 1}</div>
          <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="rounded-lg border border-border bg-card px-3 py-2 text-sm disabled:opacity-40 hover:bg-muted">
            Next →
          </button>
        </div>
      </div>
