/**
 * Anchor instruction encoding (browser/server shared).
 *
 * Mirrors backend/lib/solana-anchor.js and
 * supabase/functions/_shared/anchor-encoding.ts. Three copies exist because the
 * three runtimes cannot import from one another: Node backend, Deno Edge
 * Functions, and the Vite app. The encoding is small and fully specified by the
 * on-chain program, and the merkle cross-check test guards compatibility.
 *
