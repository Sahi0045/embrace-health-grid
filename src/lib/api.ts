...
export const updateAppointmentStatus = (id: string, status: string, notes?: string | { rejectionReason?: string; suggestedSlot?: string }) =>
  apiFetch<any>(`/appointments/${encodeURIComponent(id)}/status`, {
    method: "PATCH",
    body: JSON.stringify({ status, ...(typeof notes === "object" ? notes : { notes }) }),
  });
