    ...
    <div className="rounded-xl border border-border bg-card p-4 shadow-clinical">
      ...
      <div className="flex flex-wrap gap-2 mb-4 text-[10px]">
        {(["ward", "icu", "emergency", "surgery", "radiology", "pharmacy"] as const).map((type) => (
          <div
            key={type}
            className={`flex items-center gap-1 rounded-full border px-2 py-0.5 ${typeColors[type].split(" ").slice(0, 2).join(" ")}`}
          >
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
              {zones
                ...
                .map((zone) => {
                  ...
                  const pct =
                    zone.occupancy != null && zone.capacity
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span
                          className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${statusBadge[zone.status]}`}
                        >
                          {zone.status}
                        </span>
                      </div>
