...
import { Building2, ShieldCheck, Globe, Link2, Clock, ArrowRight, Wifi, WifiOff } from "lucide-react";
...
export function FederationHospitalCard({ hospital, showActions = true, onViewDetails, onDisconnect }: FederationHospitalCardProps) {
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold ${st.badge}`}>
            <div className={`h-1.5 w-1.5 rounded-full ${st.dot}`} />
            {st.label}
          </span>
          <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize ${trustConfig[hospital.trust]}`}>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-muted p-2">
          <div className="text-base font-bold text-foreground">{hospital.sharedCredentials.toLocaleString()}</div>
          <div className="text-[10px] text-muted-foreground">Credentials</div>
        </div>
        <div className="rounded-lg bg-muted p-2">
          <div className="text-base font-bold text-foreground">{hospital.crossVerifications.toLocaleString()}</div>
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        ...
        {hospital.specialties.length > 3 && (
          <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">+{hospital.specialties.length - 3}</span>
        )}
      </div>
      ...
      {/* DID */}
      <div className="mt-2 font-mono text-[10px] text-muted-foreground/50 truncate">{hospital.did}</div>
